...
type PS3DG(length: int, width: int, height: int) =
    let systemName = "PS3DGSystem"
    ...
    let configuration = ConfigurationFactory.ParseString(@"
                            akka {
                                ...
                                cluster {
                                    seed-nodes = [""akka.tcp://PS3DGSystem@localhost:9090""]
                                }
                            }
                        ")

    let PS3DGSystem = System.create systemName (configuration)

    let structure = new Structure(length, width, height)

    let recorder = PS3DGSystem.ActorOf(Props(typeof<RecorderActor>, [| structure :> obj |]), "recorder")
    
    do PS3DGSystem.ActorOf(Props(typeof<PrinterActor>), "printer") |> ignore
    ...
    do for i in 1 .. length do
        for j in 1 .. width do
            for k in 1 .. height do
                ...
                PS3DGSystem.ActorOf(Props(typeof<PS3DWorkerActor>, [| position :> obj; structure :> obj; no :> obj |]), name) |> ignore
                no <- (no + 1)
                workerList.Add("/user/" + name)

    let mediator = DistributedPubSub.Get(PS3DGSystem).Mediator

    let broadcastRouter = PS3DGSystem.ActorOf(Props.Empty.WithRouter(new BroadcastGroup(workerList)), "broadcastRouter")
