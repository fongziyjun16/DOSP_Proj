...
type RecorderActor(numberOfWorkers: int) =
    ...
    override x.OnReceive message =
        ...
        | :? StartRumor as msg ->
            ...
            mediator <! new Send("/user/printer", "start in " + startID.ToString(), true)
...
type TaskWorkerActor(id: int, numberOfWorkers: int) =
    ...
    member x.GetRandomNeighbor() =
        ...
        if id = 1 then randomNumber <- 2
        else if id = numberOfWorkers then randomNumber <- (numberOfWorkers - 1)
        else
            let randomDirect = Random().Next(2)
            if randomDirect = 0 then 
                randomNumber <- (id - 1)
            else
                randomNumber <- (id + 1)
        randomNumber
